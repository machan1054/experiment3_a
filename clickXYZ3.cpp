...
int main(void)
{
    ...
    while(waitKey(1) != 'q'){
        ...
        float u, v, X, Y, Z, XY[3]={}, XZ[3]={}, cos, norm;
        int min;
        if(clicked){                        // if clicked,
            ...
            if(Z != 0.){
                ...
                printf("[%d] (%+6.1f,%+6.1f) => (%+6.3f, %+6.3f, %+6.3f)\n", i, u, v, X, Y, Z);
            }
           ...
           if(i == 3) {
               ...
               XY[0] = POINT[1][0] - POINT[0][0];
               XY[1] = POINT[1][1] - POINT[0][1];
               XY[2] = POINT[1][2] - POINT[0][2];
               XZ[0] = POINT[2][0] - POINT[0][0];
               XZ[1] = POINT[2][1] - POINT[0][1];
               XZ[2] = POINT[2][2] - POINT[0][2];
               
               N[j][0] = XY[1] * XZ[2] - XY[2] * XZ[1];
               N[j][1] = XY[2] * XZ[0] - XY[0] * XZ[2];
               N[j][2] = XY[0] * XZ[1] - XY[1] * XZ[0];
               
               norm = sqrt(N[j][0]*N[j][0] + N[j][1]*N[j][1] + N[j][2]*N[j][2]);
           ...
           if(j == 2) {
               cos = -abs((N[0][0] * N[1][0] + N[0][1] * N[1][1] + N[0][2] * N[1][2]) / (sqrt(N[0][0]*N[0][0] + N[0][1]*N[0][1] + N[0][2]*N[0][2]) * sqrt(N[1][0]*N[1][0] + N[1][1]*N[1][1] + N[1][2]*N[1][2])));
               printf("cosθ= %+6.3f, θ= %+6.3fπ rad = %+6.3f degree\n", cos, acos(cos) / M_PI, acos(cos) / 2 / M_PI * 360);
               j = 0;
           }
       }
